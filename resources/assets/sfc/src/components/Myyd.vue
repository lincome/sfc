<template>
    <div>我的预定
        ...
        <scroller lock-x scrollbar-y use-pullup :pullup-config="pullupConfig2" height="-200" ref="demo3" @on-pullup-loading="load3" v-model="demo3Value">
            <div class="box2">
                <div v-for="vo in item">
                  <group>
                     <cell>{{vo.start_date}},{{getName(vo.city_start)}}->{{getName(vo.city_end)}}</cell>
                   </group>
                </div>
            </div>
        </scroller>
...
<script>
    ...
    return value2name(value, ChinaAddressData)
            ...
            fetchData(){
                var resource = this.$resource(APIURL+'users/ckyd_lists', {
                    //                    include: ''
                });
                // get item
                resource.get({
